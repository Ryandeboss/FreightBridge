...
from scripts.acceptance import milestone25
...
def make_acceptance(*, ui_acceptance, lab_ready: bool = True) -> milestone25.Milestone25Acceptance:
  acceptance = milestone25.Milestone25Acceptance(
    config=config(),
...
def test_milestone25_requires_operations_token() -> None:
  ...
  with pytest.raises(AcceptanceFailure, match='OPERATIONS_API_BEARER_TOKEN'):
    milestone25.Milestone25Acceptance(
      config=bad_config,
      analyst_ui_base_url='https://ui.example',
      recorder=StepRecorder(),
    )
...
def test_milestone25_requires_analyst_ui_base_url() -> None:
...
def test_milestone25_runs_readiness_before_browser_acceptance() -> None:
  events: list[str] = []
...
def test_milestone25_fails_when_full_lifecycle_scenario_is_missing() -> None:
...
def test_lifecycle_timeout_reports_premature_debrief_detection() -> None:
  class PageWithoutDebrief:
    ...
    def wait_for_timeout(self, *_args, **_kwargs):
      raise AcceptanceFailure('Training lifecycle', 'forced timeout')

  with pytest.raises(AcceptanceFailure, match='forced timeout'):
    milestone25.run_training_lifecycle(PageWithoutDebrief())
