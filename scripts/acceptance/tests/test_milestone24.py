...
from scripts.acceptance import milestone24
...
def make_acceptance(*, ui_acceptance, lab_ready: bool = True) -> milestone24.Milestone24Acceptance:
  acceptance = milestone24.Milestone24Acceptance(
    config=config(),
...
def test_milestone24_requires_operations_token() -> None:
  ...
  with pytest.raises(AcceptanceFailure, match='OPERATIONS_API_BEARER_TOKEN'):
    milestone24.Milestone24Acceptance(
      config=bad_config,
      analyst_ui_base_url='https://ui.example',
      recorder=StepRecorder(),
    )
...
def test_milestone24_requires_analyst_ui_base_url() -> None:
...
def test_milestone24_runs_readiness_before_browser_acceptance() -> None:
  events: list[str] = []
...
def test_milestone24_fails_when_full_lifecycle_scenario_is_missing() -> None:
