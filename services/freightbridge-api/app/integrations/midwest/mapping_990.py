...
def map_midwest_990(interchange: X12Interchange) -> Midwest990MappingResult:
  ...
  if decision_code == 'A':
    ...
    raise Midwest990MappingError('UNSUPPORTED_TENDER_DECISION', 'Unsupported Midwest 990 decision code.')
  ...
  carrier_load_number = references.get('CN')
  reason_code = references.get('ZZ')
  ...
  return Midwest990MappingResult(
    ...
    bol_reference=references.get('BM'),
    po_reference=references.get('PO'),
...
def _validate_profile(interchange: X12Interchange) -> None:
  ...
  if interchange.isa_segment.element(6).strip() != 'MWCX' or interchange.isa_segment.element(8).strip() != 'FREIGHTBRIDGE':
    raise Midwest990MappingError('INVALID_PARTNER_PROFILE', 'Unexpected Midwest 990 sender or receiver.')
  group = interchange.functional_groups[0]
  transaction = group.transaction_sets[0]
  if group.functional_identifier != 'GF' or group.version_release != '004010':
    raise Midwest990MappingError('INVALID_PARTNER_PROFILE', 'Unexpected Midwest 990 functional group.')
  if transaction.transaction_set_identifier != '990':
