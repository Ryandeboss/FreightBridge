# Partner Documentation

This folder contains synthetic partner contracts for the FreightBridge portfolio lab.

## Apex Logistics

- [Partner profile](apex/partner-profile.md)
- [Data dictionary](apex/data-dictionary.md)
- [OpenAPI contract](apex/openapi.yaml)
- [Outbound load tender contract](apex/outbound-load-tender-contract.md)
- [Service catalog](apex/service-catalog.md)

## Midwest Carrier

- [Partner profile](midwest/partner-profile.md)
- [Data dictionary](midwest/data-dictionary.md)
- [EDI implementation guide](midwest/edi-implementation-guide.md)
- [Connectivity specification](midwest/connectivity-specification.md)
- [Service catalog](midwest/service-catalog.md)

## Cross-Partner Contracts

- [Trading partner matrix](trading-partner-matrix.md)
- [Error contract](error-contract.md)

No partner simulator behavior, canonical mapping, EDI parsing, SFTP exchange, or database domain model is implemented in this milestone.
